    ...
    describe DatadogPlugin do
      context "without proxy plugin" do
        ...
        it "gets hooked with container_instance_user_data trigger" do
          ...
          expect(user_data["runcmd"].last).to eq "docker run -d --name dd-agent -h `hostname` -v /var/run/docker.sock:/var/run/docker.sock -v /proc/:/host/proc/:ro -v /sys/fs/cgroup/:/host/sys/fs/cgroup:ro -e API_KEY=abcdef TAGS=\"barcelona,district:dev\" datadog/docer-dd-agent:latest"
        end
      end
