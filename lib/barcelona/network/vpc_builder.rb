  ...
  module Network
    class VPCBuilder < CloudFormation::Builder
      ...
      def build_resources
        ...
        stack.district.notifications.each do |notification|
          case notification["target"]
          when "slack"
            add_resource("AWS::SNS::Subscription", "SlackSubscription") do |j|
              j.Endpoint get_attr("SlackNotification", "Arn")
              j.Protocol "lambda"
              j.TopicArn ref("NotificationTopic")
            end

            add_resource("AWS::Lambda::Permission", "NotificationPermission") do |j|
              j.FunctionName ref("SlackNotification")
              j.Action "lambda:InvokeFunction"
              j.Principal "sns.amazonaws.com"
            end

            add_resource("AWS::Lambda::Function", "SlackNotification") do |j|
